...
const ProductInfo: React.FC<ProductInfoProps> = ({ data }) => {
  ...
  const [sizeType, setSizeType] = useState("us"); // Используем 'us' как начальное значение

  const [selectedSizeIndex, setSelectedSizeIndex] = useState(0);
  const [entrySize, setEntrySize] = useState(variationsData[0].id);
  ...
  const handleSizeTypeChange = (newSizeType: string) => {
    ...
    setEntrySize(variationsData[selectedSizeIndex]);
  };
      ...
      <div>
          ...
          <div className="flex items-center gap-2">
            <div>
              <input
                ...
                id="us"
                checked={sizeType === "us"}
                onChange={() => handleSizeTypeChange("us")}
              />
            ...
            <div>
              <input
                ...
                id="uk"
                checked={sizeType === "uk"}
                onChange={() => handleSizeTypeChange("uk")}
              />
            ...
            <div>
              <input
                ...
                name="size-type"
                id="rus"
                checked={sizeType === "rus"}
                onChange={() => handleSizeTypeChange("rus")}
              />
        ...
        <div className={styles.sizes}>
          {variationsData?.map((item, index) => (
            <div
              key={item.id}
              className={`${styles.item} ${!!item.stock_status && styles.instock}`}
